...
def test_build(cli_output):
    ...
    result = subprocess.run(
        ["python", "-m", "build", "--sdist", "--wheel"], capture_output=True, check=True, cwd=package_path, text=True
    )
...
def test_lint(cli_output):
    ...
    result = subprocess.run(["ruff", "check", package_path], capture_output=True, check=True, text=True)
    assert result.returncode == 0
...
def test_format(cli_output):
    ...
    result = subprocess.run(["ruff", "format", "--check", package_path], capture_output=True, check=True, text=True)
    assert result.returncode == 0
...
def test_logging(cli_output):
    ...
    result = subprocess.run(["python", "log_test.py"], capture_output=True, check=True, cwd=module_path, text=True)
